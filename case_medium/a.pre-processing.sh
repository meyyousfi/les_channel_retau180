#!/bin/bash
# Pre-processing for the MEDIUM (96^3) mesh.
# Initial condition: the final (latest) solution of case_coarse, interpolated onto this
# mesh with mapFields. case_coarse must therefore have been run AND reconstructed first.
cd "${0%/*}" || exit 1
. ${WM_PROJECT_DIR:?}/bin/tools/RunFunctions

SOURCE_CASE=../case_coarse
